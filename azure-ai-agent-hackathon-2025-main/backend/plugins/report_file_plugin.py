...
class ReportFilePlugin:
    ...
    def _log_report_to_database(self, session_id: str, conversation_id: str, 
        ...
        """
        try:
            insert_table_with_retry('fact_risk_report', {
                'session_id': session_id,
                'conversation_id': conversation_id,
                'filename': filename,
                'blob_url': blob_url,
    ...
    def generate_report_from_conversation(self, conversation_id: str, session_id: str, report_type: str = "comprehensive") -> str:
        ...
        try:
            ...
            response = client.table('dim_agent_event_log').select(
                'agent_name, action, event_time, user_query, agent_output, result_summary'
            ).eq('conversation_id', conversation_id).order('event_time').execute()
    ...
    def get_reports(self, session_id: str = None, conversation_id: str = None) -> str:
        ...
        try:
            client = get_connection()
            query = client.table('fact_risk_report').select('*')
            ...
            response = query.order('created_date', desc=True).execute()
...
def save_report_to_file(report_content: str, session_id: str, conversation_id: str, report_title: str = None) -> str:
    ...
    try:
        ...
        return plugin.save_report_to_file(report_content, session_id, conversation_id, report_title)
    except Exception as e:
